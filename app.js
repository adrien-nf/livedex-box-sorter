...
function getMaxBox(dexName, dexMap) {
  if (getBoxLayout() === 'national' && dexMap) {
...
function buildGrid(dexMap, box, activeSlot, dexName, onCellClick) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    cell.className = 'box-cell' + (isActive ? ' active' : '') + (isCaught ? ' caught' : '');
